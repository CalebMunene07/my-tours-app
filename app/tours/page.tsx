    ...
    >
              ...
              <div className="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100">
                ...
                <div className="p-6">
                  ...
                  <div className="mt-5 flex items-center justify-between">
                    {/* ✅ FIXED: was tour.pricing[0].price — now uses priceUSD */}
                    <span className="text-2xl font-black text-gray-900">
                      {tour.pricing[0].priceUSD}
                    </span>
      ...
      <section className="mb-24">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {[
            { destination: "Dubai, UAE",     flag: "🇦🇪", tagline: "City of Gold",         highlight: "Burj Khalifa · Desert Safari · Gold Souk",         description: "Experience the world's most dazzling skyline by day and a magical desert camp under the stars by night. Shop the Gold Souk, dine on the 124th floor, and ride dunes at sunset — Dubai is pure spectacle.",                                                                               duration: "5–7 Days",  price: "From $1,400", gradient: "from-amber-900/80 to-yellow-700/60",  emoji: "🏙️" },
            { destination: "Maldives",       flag: "🇲🇻", tagline: "The Last Paradise",     highlight: "Overwater Villas · Coral Reefs · Bioluminescence",   description: "Wake up above turquoise lagoons in a private overwater villa. Snorkel pristine coral gardens, watch glowing bioluminescent shores at midnight, and indulge in spa rituals with the ocean as your backdrop.",                                                                       duration: "6–10 Days", price: "From $2,200", gradient: "from-cyan-900/80 to-teal-600/60",    emoji: "🌊" },
            { destination: "Istanbul, Turkey", flag: "🇹🇷", tagline: "Where Worlds Meet",  highlight: "Hagia Sophia · Bosphorus · Grand Bazaar",             description: "Straddle two continents in a city of spice, mosques, and history. Cruise the Bosphorus at dusk, lose yourself in the 4,000-shop Grand Bazaar, and marvel at Hagia Sophia's 1,500 years of stories.",                                                                          duration: "5–8 Days",  price: "From $1,100", gradient: "from-red-900/80 to-rose-700/60",     emoji: "🕌" },
            { destination: "Bali, Indonesia", flag: "🇮🇩", tagline: "Island of the Gods",  highlight: "Rice Terraces · Temples · Luxury Villas",             description: "Immerse yourself in emerald rice terraces, centuries-old Hindu temples, and world-class wellness retreats. Watch sunrise at Mount Batur, catch waves at Seminyak, and feast on Balinese cuisine under the palms.",                                                              duration: "7–10 Days", price: "From $1,600", gradient: "from-green-900/80 to-emerald-700/60", emoji: "🌴" },
            { destination: "Paris, France",  flag: "🇫🇷", tagline: "The City of Light",    highlight: "Eiffel Tower · Louvre · French Riviera",              description: "Stroll boulevards lined with chestnut trees, sip café crème at a pavement bistro, and stand before the Mona Lisa. Then head south for a week of lavender fields, rosé wine, and the glittering Côte d'Azur.",                                                               duration: "6–9 Days",  price: "From $2,500", gradient: "from-slate-800/80 to-blue-700/60",   emoji: "🗼" },
            { destination: "Egypt",          flag: "🇪🇬", tagline: "Land of the Pharaohs", highlight: "Pyramids of Giza · Nile Cruise · Luxor",              description: "Stand before the last surviving Wonder of the Ancient World, sail a felucca down the world's longest river at golden hour, and explore the Valley of the Kings — where pharaohs sleep for eternity.",                                                                    duration: "7–10 Days", price: "From $1,300", gradient: "from-yellow-900/80 to-orange-700/60", emoji: "🏺" },
            { destination: "Thailand",       flag: "🇹🇭", tagline: "The Land of Smiles",   highlight: "Bangkok · Phi Phi Islands · Chiang Mai",              description: "Float through Bangkok's electric street markets, island-hop the Andaman's emerald limestone cliffs, and ride an elephant through misty Chiang Mai jungle — Thailand dazzles at every turn.",                                                                                   duration: "8–12 Days", price: "From $1,800", gradient: "from-purple-900/80 to-pink-700/60",  emoji: "🐘" },
            { destination: "Rome, Italy",    flag: "🇮🇹", tagline: "The Eternal City",     highlight: "Colosseum · Vatican · Amalfi Coast",                  description: "Toss a coin in the Trevi Fountain, walk through 2,000 years of history at the Colosseum, and savour handmade pasta in a candlelit trattoria. Extend to the clifftop Amalfi Coast for pure Mediterranean magic.",                                                           duration: "6–9 Days",  price: "From $2,100", gradient: "from-orange-900/80 to-red-700/60",   emoji: "🏛️" },
          ].map((place) => (
            <Link key={place.destination} href="/contact"
              ...
              <div className={`absolute inset-0 bg-linear-to-t ${place.gradient} opacity-90 group-hover:opacity-100 transition-opacity duration-300`} />
              ...
              <div className="relative z-10 p-6 group-hover:opacity-0 group-hover:translate-y-2 transition-all duration-300">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-white/60 text-[10px] font-bold tracking-[0.2em] uppercase">{place.tagline}</span>
                </div>
                <h3 className="text-xl font-black text-white mb-1 leading-tight">{place.destination}</h3>
                <p className="text-white/60 text-xs leading-relaxed mb-4">{place.highlight}</p>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[#D4AF37] font-black text-base">{place.price}</p>
                    <p className="text-white/40 text-[10px]">{place.duration}</p>
                  </div>
                  <span className="bg-white/10 text-white text-[10px] font-bold tracking-widest uppercase px-3 py-2 rounded-full">Inquire</span>
                </div>
              </div>
              {/* Hover */}
              <div className="absolute inset-0 z-20 flex flex-col justify-end p-6 opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <div>
                    <h3 className="text-lg font-black text-white leading-tight">{place.destination}</h3>
                    <span className="text-[#D4AF37] text-[10px] font-bold tracking-[0.2em] uppercase">{place.tagline}</span>
                  </div>
                </div>
                <p className="text-white/85 text-xs leading-relaxed mb-5">{place.description}</p>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[#D4AF37] font-black text-sm">{place.price}</p>
                    <p className="text-white/50 text-[10px]">{place.duration}</p>
                  </div>
                  <span className="bg-[#D4AF37] text-[#1a1208] text-[10px] font-black tracking-widest uppercase px-4 py-2 rounded-full">Inquire →</span>
                </div>
              </div>
            </Link>
          ))}
      ...
      <section className="pt-16 border-t border-gray-100">
        <div className="text-center mb-16">
          ...
          <p className="text-gray-500 max-w-xl mx-auto leading-relaxed">
            Four distinct tiers of service — from essential comfort to bespoke romantic escapes — ensuring your safari matches your vision perfectly.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
          {packages.map((pkg) => (
            <div key={pkg.name}
              ...
              style={{ backgroundColor: pkg.romantic ? "#1a0a12" : "#4B5320", border: `3px solid ${pkg.romantic ? "#c8729a" : "#D4AF37"}` }}>
              <div className="px-8 pt-8 pb-6 border-b"
                style={{ borderColor: pkg.romantic ? "rgba(200,114,154,0.2)" : "rgba(255,255,255,0.1)" }}>
                <div className="mb-4">{pkg.romantic ? <Heart size={36} className="text-[#c8729a]" /> : pkg.icon}</div>
                <h3 className="text-2xl font-black mb-1" style={{ color: pkg.romantic ? "#c8729a" : "#D4AF37" }}>{pkg.name}</h3>
                ...
                {pkg.popular && (
                  <span className="inline-block mt-3 text-[9px] font-bold tracking-[0.18em] uppercase bg-[#D4AF37] text-[#4B5320] px-3 py-1 rounded-full">Most Popular</span>
                )}
                {pkg.romantic && (
                  <span className="inline-block mt-3 text-[9px] font-bold tracking-[0.18em] uppercase px-3 py-1 rounded-full" style={{ background: "#c8729a", color: "#fff" }}>💍 Special Occasions</span>
                )}
              </div>
                ...
                <ul className="space-y-3 mb-8 flex-1">
                  {pkg.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-3 text-sm text-white/85 leading-snug">
                      <CheckCircle2 size={16} className="shrink-0 mt-0.5" style={{ color: pkg.romantic ? "#c8729a" : "#D4AF37" }} />
                      {feature}
                    </li>
                  ))}
                </ul>
